...
We think this architecture could have profound implications throughout the entire UI development stack. In the Riffle project, we're interested in exploring these implications in the broadest sense. Most client-side web UI technologies have been developed in a context where data is assumed to live far away on a server; now the data can be immediately close at hand on the client device. How might this architecture allow us to build simpler apps? Could we take more integrated approaches to computing with data that make it easier for developers to build, maintain, and debug their applications? Can we make apps more performant by default? Could apps become more customizable and composable by end users?
